...
import styles from './entity-explorer.css?inline';
...
export class EntityExplorer extends LitElement {
  ...
  connectedCallback(): void {
    ...
    try {
      const saved = localStorage.getItem('appbana_explorer_query');
      if (saved) {
  ...
  private setToken() {
    ...
    if (clean) localStorage.setItem('appbana_token', clean);
    else localStorage.removeItem('appbana_token');
    this.requestUpdate();
  }
  ...
  private removeFilter(id: string) {
    this.updateQuery('filters', this.query.filters.filter(f => f.id !== id));
  }
  ...
  private async doBatchInsert() {
    ...
    try {
      ...
      this.batchError = 'Parse error: ' + (e.message || String(e));
      return;
    }
    try {
  ...
  private renderFilters() {
    return html`
      <table class="filters-table">
        ...
        <tbody>
          ${this.query.filters.map(f => html`
            <tr>
              <td>
                <input
                  .value=${f.field}
                  @input=${(e: any) => this.updateFilter(f.id, 'field', e.target.value)}
                  placeholder="status" />
              </td>
              <td>
                <input
                  .value=${f.value}
                  @input=${(e: any) => this.updateFilter(f.id, 'value', e.target.value)}
                  placeholder="ACTIVE" />
              </td>
              <td class="right">
                <button
                  @click=${() => this.removeFilter(f.id)}
                  title="Remove"
          ...
          <tr>
            <td colspan="3">
              <button @click=${() => this.addFilterRow()} style="width:100%">+ Add Filter</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  private renderResultTable() {
    ...
    if (!rows.length) {
      return html`<div class="small muted">${this.loadingData ? 'Loading...' : 'No rows.'}</div>`;
    }
    ...
    return html`
      <div style="overflow:auto; max-height:420px; border:1px solid #e1e4e9; border-radius:4px;">
        <table>
          ...
          <tbody>
            ${rows.map(r => html`
              <tr>
                ${headers.map(h => html`<td class="mono">${this.formatCell(r[h])}</td>`)}
              </tr>
            `)}
          </tbody>
        </table>
      ...
      <div class="small" style="margin-top:6px; display:flex; justify-content:space-between; align-items:center;">
        <div>
          ${this.result && !Array.isArray(this.result) && this.result.total != null
            ? html`<span>Total: <strong>${this.result.total}</strong></span>`
            : ''}
      ...
      <div class="grid">
        <div class="panel">
          ...
          <select
            ...
            @change=${(e: any) => {
              this.updateQuery('entity', e.target.value);
              this.resetPaging();
            }}>
          ...
          <div class="row" style="margin-top:8px;">
            <div class="grow">
              ...
              <input
                .value=${this.query.q}
                @input=${(e: any) => {
                  this.updateQuery('q', e.target.value);
                  this.resetPaging();
                }}
            ...
            <div>
              ...
              <input
                type="number"
                style="width:90px"
                .value=${String(this.query.limit)}
                @input=${(e: any) => this.updateQuery('limit', Math.max(1, Math.min(500, Number(e.target.value) || 50)))} />
            </div>
            <div>
              ...
              <input
                type="number"
                style="width:90px"
                .value=${String(this.query.offset)}
                @input=${(e: any) => this.updateQuery('offset', Math.max(0, Number(e.target.value) || 0))} />
            </div>
          </div>
          ...
          <div class="row" style="margin-top:8px;">
            <div class="grow">
              ...
              <input
                ...
                @input=${(e: any) => {
                  this.updateQuery('fields', e.target.value);
                  this.resetPaging();
                }}
          ...
          <div class="row" style="margin-top:8px;">
            <div class="grow">
              ...
              <input
                ...
                @input=${(e: any) => {
                  this.updateQuery('sort', e.target.value);
                  this.resetPaging();
                }}
                placeholder="-createdAt,firstName" />
            </div>
            <div style="display:flex; flex-direction:column; justify-content:flex-end;">
              ...
              <label style="font-size:12px; display:flex; align-items:center; gap:4px;">
                <input
                  type="checkbox"
                  .checked=${this.query.count}
                  @change=${(e: any) => this.updateQuery('count', e.target.checked)} />
                count-only
              </label>
            </div>
          </div>
          ...
          <div class="row" style="justify-content:space-between;">
            <div style="display:flex; gap:6px;">
              <button
                class="primary"
                @click=${() => this.runQuery()}
                ?disabled=${!this.query.entity || this.loadingData}>
                ${this.loadingData ? 'Running...' : 'Run Query'}
              </button>
              <button
                @click=${() => {
                  this.updateQuery('offset', 0);
            ...
            <button @click=${() => {
              this.updateQuery('q', '');
              this.updateQuery('fields', '');
              this.updateQuery('sort', '');
              this.updateQuery('filters', []);
              this.updateQuery('count', false);
              this.updateQuery('offset', 0);
            }}>
              Clear
            </button>
          ...
          <textarea
            style="min-height:120px; font-family:monospace; font-size:11px;"
            .value=${this.batchInput}
            @input=${(e: any) => this.batchInput = e.target.value}>
          </textarea>
          <div class="row" style="margin-top:6px; align-items:center;">
            <button @click=${() => this.doBatchInsert()} ?disabled=${!this.query.entity}>
              Insert Batch
            </button>
            ${this.batchError ? html`<span class="error">${this.batchError}</span>` : ''}
            ${this.batchResult && !this.batchError
              ? html`<span class="success">Inserted: ${this.batchResult.inserted ?? '?'}</span>`
              : ''}
          </div>
        </div>
        ...
        <div class="panel">
          ...
          ${this.query.count && this.result && !Array.isArray(this.result) && this.result.total != null
            ? html`<div class="pill" style="margin-bottom:8px;">Count: ${this.result.total}</div>`
            : ''}
          ...
          ${this.batchResult
            ? html`
              <details style="margin-top:12px;">
                <summary class="small">Batch Insert Response</summary>
                <pre>${JSON.stringify(this.batchResult, null, 2)}</pre>
              </details>`
            : ''}
        </div>
      </div>
