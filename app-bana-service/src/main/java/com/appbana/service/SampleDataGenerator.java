...
public class SampleDataGenerator {
    ...
    private void insertRecord(String tableName, Map<String, Object> record) throws SQLException {
        ...
        try (Connection conn = JdbcManager.getConnection();
            ...
            for (String column : columns) {
                ps.setObject(paramIndex++, hashIfPasswordColumn(column, record.get(column)));
            }
    ...
    /**
     * S4.2 — this class builds and executes its own raw INSERT independently of
     * {@code EntityCrudService} (it predates it and is only ever invoked, via
     * {@code AppPublishService}, for LOCAL-environment sample-data seeding), so it needs its own
     * hash-on-write guard rather than inheriting {@code EntityCrudService}'s. A column literally
     * named "password" (case-insensitive — same convention as
     * {@code GenericAppAuthController.login()}'s exact-match column check) generated by
     * {@link #generateTextValue} falls through to the generic {@code "Sample " + label + " " +
     * index} branch (no field-name pattern here matches "password"), which would otherwise be
     * persisted completely in the clear. Idempotent like every other S4.2 write path: a value
     * that already looks like a BCrypt hash is passed through unchanged rather than re-hashed.
     */
    private static Object hashIfPasswordColumn(String column, Object value) {
        if ("password".equalsIgnoreCase(column) && value instanceof String s && !s.isEmpty()
                && !PasswordService.looksLikeBcryptHash(s)) {
            return PasswordService.hashPassword(s);
        }
        return value;
    }
}
