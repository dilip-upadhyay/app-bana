...
public class WorkflowEngine {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                advanceFromNode(conn, instanceId, startNodeId, nodes, workflowJson,
                        entityType, entityData, new HashMap<>());
    ...
    public void completeTask(String tokenId, String outcome, String taskData, String userId) {
        try (Connection conn = dataSource.getConnection()) {
            ...
            try {
                ...
                // 5. Advance to next node (pass outcome for transition condition evaluation)
                Map<String, Object> taskContext = new HashMap<>();
                taskContext.put("outcome", outcome);
                if (taskData != null) {
                    try {
                        Map<String, Object> parsedTaskData = objectMapper.readValue(
                                taskData,
                                new TypeReference<Map<String, Object>>() {
                                });
                        taskContext.putAll(parsedTaskData);
                    } catch (Exception e) {
                        log.warn("Failed to parse task data: {}", taskData);
                    }
                }

                advanceFromNode(conn, instance.getId(), token.getNodeId(), nodes, workflowJson,
                        instance.getEntityType(), entityData, taskContext);
     ...
     * 
     * @param taskContext Additional context from completed task (e.g., outcome,
     *                    form data)
     */
    private void advanceFromNode(Connection conn, String instanceId, String fromNodeId,
            ...
            String entityType, Map<String, Object> entityData,
            Map<String, Object> taskContext) throws Exception {
        ...
        // Add task context (outcome, form data) for transition condition evaluation
        if (taskContext != null) {
            context.putAll(taskContext);
        }
        for (Map<String, Object> transition : outgoing) {
            ...
            if (condition == null || ExpressionEvaluator.evaluateCondition(condition, context)) {
                ...
                if ("END".equals(nodeType)) {
                    ...
                    // Continue advancing (recursive) - no task context for service tasks
                    advanceFromNode(conn, instanceId, toNodeId, nodes, workflowJson, entityType, entityData,
                            new HashMap<>());

                } else if ("DECISION".equals(nodeType)) {
                    // Decision nodes have no token, just route (preserve task context)
                    advanceFromNode(conn, instanceId, toNodeId, nodes, workflowJson, entityType, entityData,
                            taskContext);
     ...
     */
    private void createToken(Connection conn, String instanceId, String nodeId, String nodeType, String status)
            throws SQLException {
        String tokenId = UUID.randomUUID().toString();
        String sql = """
                INSERT INTO appbana_wf_token
    ...
    private WorkflowDefinition loadWorkflowDefinitionByInstanceVersion(Connection conn, String defId, int version)
            throws SQLException {
    ...
    private WorkflowDefinition mapWorkflowDefinition(ResultSet rs) throws SQLException {
        ...
        if (createdAt != null)
            def.setCreatedAt(createdAt.toLocalDateTime());

        Timestamp updatedAt = rs.getTimestamp("updated_at");
        if (updatedAt != null)
            def.setUpdatedAt(updatedAt.toLocalDateTime());
    ...
    private WorkflowInstance mapWorkflowInstance(ResultSet rs) throws SQLException {
        ...
        if (startedAt != null)
            inst.setStartedAt(startedAt.toLocalDateTime());

        Timestamp completedAt = rs.getTimestamp("completed_at");
        if (completedAt != null)
            inst.setCompletedAt(completedAt.toLocalDateTime());
    ...
    private WorkflowToken mapWorkflowToken(ResultSet rs) throws SQLException {
        ...
        if (nodeType != null)
            token.setNodeType(WorkflowToken.NodeType.valueOf(nodeType));

        String status = rs.getString("status");
        if (status != null)
            token.setStatus(WorkflowToken.TokenStatus.valueOf(status));

        String assignmentType = rs.getString("assignment_type");
        if (assignmentType != null)
            token.setAssignmentType(WorkflowToken.AssignmentType.valueOf(assignmentType));
        ...
        if (dueAt != null)
            token.setDueAt(dueAt.toLocalDateTime());

        String slaStatus = rs.getString("sla_status");
        if (slaStatus != null)
            token.setSlaStatus(WorkflowToken.SlaStatus.valueOf(slaStatus));

        Timestamp arrivedAt = rs.getTimestamp("arrived_at");
        if (arrivedAt != null)
            token.setArrivedAt(arrivedAt.toLocalDateTime());

        Timestamp completedAt = rs.getTimestamp("completed_at");
        if (completedAt != null)
            token.setCompletedAt(completedAt.toLocalDateTime());
