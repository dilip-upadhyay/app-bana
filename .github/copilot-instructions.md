...
> [!WARNING]
> **A brand-new test that's the first to ever exercise a given route can surface a real, previously-invisible resource leak — don't assume a reproducible full-suite failure it causes is the test's own fault.** `S1.11`'s `CrossTenantAppAccessTest` was the first automated test anywhere to call `POST /api/{tenantId}/apps/{id}/versions` as a legitimate (guard-admitted) caller. Doing so reliably (2/2 runs) failed an unrelated test, `RevisionFlowTest.concurrentPutsOnTheSameParentProduceOnlyOneRevision`, with a `SQLTransientConnectionException` (HikariCP pool exhausted — `active=10, idle=0`). Root cause: `ReleaseService.createVersion()` acquired its `Connection` as a bare local (`Connection conn = JdbcManager.getConnection();`) with no try-with-resources and no explicit close, on every call — the only such unwrapped call site among 65 `JdbcManager.getConnection(` uses in `src/main` (confirmed by a repo-wide grep before concluding it was isolated). Manual/live verification calls across prior sessions never accumulated enough volume to exhaust a 10-connection pool that gets a fresh JVM per dev session; a test suite calling the same leaking route even once, in the same JVM as 400+ other tests, was enough. **Before blaming a new test for a full-suite failure it seems to cause, isolate**: run the newly-added test(s) alone (rules out a logic bug in the test itself), and re-run the full suite with the new test(s) excluded (confirms whether the failure reproduces without them) — only once both point at the new test's own code path is it fair to dig into what that code path does differently, which is what actually led to the fix here (wrap the connection in try-with-resources, matching every sibling method in the same file).
