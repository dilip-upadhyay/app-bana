...
- **Empty-request gap**: a parameter-by-parameter sweep tests every value of every parameter but never tests the absence of all of them. A handler that branches on "did the caller send anything" (`if (anyAdv) {...} else {...}` or equivalent) often takes a structurally different code path — a bare request with no query string at all — that shares no parameter with the matrix and so is never triggered by testing parameter values, however exhaustively. Probe the empty request as its own explicit case, every time a handler has that branch shape.
- **Sibling caller of the method you changed**: verifying the call site named in the defect report is not the same as verifying every caller of the method that was actually edited. A fix that lands in a shared method rather than the calling route can fix an uninvolved sibling call site for free — or leave one broken, if the method isn't as shared as assumed. Enumerate every caller of the changed method and probe each independently; do not assume they share behavior just because they share code.
- **The failure mode narrowing introduces**: replacing a broad/permissive read (`SELECT *`, an unfiltered list, a catch-all) with an explicit, narrower one changes what was silently degraded into what can now fail loudly — a declared field absent from the physical table, a type the explicit list assumes but the row doesn't have, a key the caller expected that the narrower set no longer includes. When a fix narrows what a query, filter, or projection returns, deliberately construct the case where the narrowed set and the physical/actual reality disagree, and confirm it degrades the same way it did before, not worse.
