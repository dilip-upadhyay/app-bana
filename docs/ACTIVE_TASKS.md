...
| `ai-builder` | `mvn -B verify` | 168 tests · 0 failures · 0 errors · 2 skipped |
...
**ai-builder count correction (2026-07-29):** this table read `145 tests · 0 failures` for several rounds
while the suite was actually running 162 with **2 failures**. `KnowledgeBaseServiceIntegrationTest`
asserted `assertEquals(39, getIndexedCount())` against a RAG corpus that had since grown to 47, so the
suite had been red without anyone noticing — precisely the gap the CI fix above closes. The assertion now
...
templates. Count is now 168 (162 + 6 new C4 tests), 0 failures.
