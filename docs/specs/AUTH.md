...
> Reference implementation lives in [`app-bana-studio/src/features/auth/AuthGate.tsx`](../../app-bana-studio/src/features/auth/AuthGate.tsx). The gate handles login, session-expired recovery (listens for the `appbana:auth:expired` browser event dispatched by [`app-bana-shared/src/api-client.ts`](../../app-bana-shared/src/api-client.ts) `authedFetch`), and mounts the studio only once a session is present.

Session state persists in `localStorage` under `appbana_session`. On mount, the gate rehydrates the session; on 401 from any backend call, it clears the session and re-shows the login screen with a "your session expired" banner.
...
- [ ] Build `AuthGate.tsx` in [`app-bana-studio/src/features/auth/`](../../app-bana-studio/src/features/auth/) (login + register + session-expired banner)
- [ ] Wire `authedFetch` in [`app-bana-shared/src/api-client.ts`](../../app-bana-shared/src/api-client.ts) to emit `appbana:auth:expired` on 401
- [ ] Persist session token in `localStorage.appbana_session`
- [ ] Add protected-route wrapper that mounts `AuthGate` until a session is present
- [ ] Test login / register / session-expired flow in browser
