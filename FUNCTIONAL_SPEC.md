...
Purpose
- Describe current, working functionality of the AppBana MVP (metadata-driven UI → API → DB), including multi-datasource management, and provide prioritized future enhancements.
...
- Frontend: static HTML/JS UIs served from resources:
  ...
  - datasource.html — manage datasources (add/update/list/activate/delete) via /ui/datasource/* endpoints
- Backend services:
  - ApiServer — embedded HTTP server; handlers for /schema, /api/*, /openapi.json, and datasource routes under /ui/datasource/*
  ...
  - JdbcManager — resolves the active datasource from config, infers JDBC driver from type/URL when missing, ensures metadata tables exist
...
- Datasource management (JSON):
  - GET /ui/datasource/list — list all datasources (no passwords)
  - GET /ui/datasource/config — current active datasource details (no password)
  - POST /ui/datasource/save — upsert datasource and set active
    - Body: { name, type?, url, username?, password?, driver? }
...
- AppConfig now supports multi-datasource:
  - datasources: [ { name, type, jdbcUrl, username, password, driver } ]
...
- Same as v1.0; additionally secure datasource endpoints and protect secrets; consider per-datasource health checks
...
- Key files:
  - src/main/java/org/example/ApiServer.java — adds /ui/datasource/* endpoints
  - src/main/java/org/example/JdbcManager.java — resolves active datasource, infers driver
  ...
  - src/main/resources/ui/datasource.html — UI for datasource management
- Built artifacts: target/app-bana-1.0-SNAPSHOT-fat.jar

13. Recommended enhancements (prioritized)
- A: AuthN/AuthZ for schema and datasource management; connection pooling (HikariCP); health checks
